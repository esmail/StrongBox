...
class TestDirectoryMerkleTree(unittest.TestCase):
  test_dir = '.test_temp'
  ...
  def setUpClass(cls):
    ...
    tree_original = DirectoryMerkleTree.make_dmt(cls.original_dir)
    tree_modified = DirectoryMerkleTree.make_dmt(cls.modified_dir)
  ...
  def test_hash_determinism(self):
    """
    Check that a directory produces the same hash when a Merkle tree is made
    from it twice in a row.
    """
    hash_original = tree_original.dmt_hash
    hash_original_regenerated = DirectoryMerkleTree.make_dmt(self.original_dir).dmt_hash
  ...
  def test_tree_determinism(self):
    ...
    tree_original_regenerated = DirectoryMerkleTree.make_dmt(self.original_dir)
  ...
  def test_tree_difference_unmodified(self):
    updated, new, deleted = DirectoryMerkleTree.compute_tree_changes(tree_modified, tree_original)
  ...
  def test_tree_difference_updated(self):
    updated, _, _ = DirectoryMerkleTree.compute_tree_changes(tree_modified, tree_original)
  ...
  def test_tree_difference_new(self):
    _, new, _ = DirectoryMerkleTree.compute_tree_changes(tree_modified, tree_original)
    self.assertEqual(new, self.new_files.union(new_dirs), 'Error in calculating new items.')

  def test_tree_difference_deleted(self):
    _, _, deleted = DirectoryMerkleTree.compute_tree_changes(tree_modified, tree_original)
